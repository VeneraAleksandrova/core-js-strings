...
function countVowels(str) {
  return str.toLowerCase().match(/[aeiouy]/g)
    ? str.toLowerCase().match(/[aeiouy]/g).length
    : 0;
}
/**
...
function findLongestWord(sentence) {
  return sentence
    ...
    .find(
      (e) =>
        e.length === Math.max(...sentence.split(' ').map((el) => el.length))
    );
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) => (alph.indexOf(el) !== -1 ? newAlph[alph.indexOf(el)] : el))
    .join('');
}
